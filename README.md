...
3. 想取得的網址數量（1 到 500）
4. 輸出檔名；若未包含 `.txt`，程式會自動補上；檔案已存在時會要求確認覆寫
...
| `--count` | 網址數量，範圍為 1 到 500 | `20` |
| `--output` | 輸出檔名或路徑 | `output/list.txt` |
| `--overwrite` | 明確允許覆寫已存在的檔案 | `--overwrite` |

四個主要參數必須同時提供。完全不提供參數時，程式會進入互動模式。命令列模式遇到既有檔案時會停止，不會直接覆寫；確定要覆寫才加上 `--overwrite`。
...
只輸入頻道名稱時，程式只會使用名稱完全相同的頻道。若找不到精準結果，程式會停止並要求輸入 `@handle` 或完整頻道網址，避免同名頻道造成誤抓。

## 注意事項

- 若搜尋到錯誤頻道，請改用 `@handle` 或完整頻道首頁網址。
- 取得數量不足通常代表頻道內容不足，或部分內容無法公開讀取；完全沒有可用網址時不會建立空白檔案。
...
測試不會連線到 YouTube，而是以模擬回應驗證頻道辨識、網址整理、數量限制、覆寫保護與錯誤處理：
